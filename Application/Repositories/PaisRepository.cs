...
namespace Application.Repositories
{
    public class PaisRepository : GenericRepositoryInt<Pais>, IPais
    {
        ...
        public async Task<IEnumerable<Pais>> GetPaisesConEstadosAsync()
        {
            return await _context.Paises
                .Include(p => p.Regiones)  // Incluye las regiones relacionadas
                    // .ThenInclude(r => r.Ciudades)  // Opcionalmente, puedes incluir más niveles
                    //     .ThenInclude(c => c.Direcciones)
                    //         .ThenInclude(d => d.Clientes)
                .ToListAsync();
        }
